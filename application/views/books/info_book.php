				...
				<div class="row">
					...
					<div class="col-sm-7 col-xs-12">
						<div class="row">
							<div class="col-sm-12">
								<p>
									<strong>Titulo</strong>: <?= $book->book_name ?></p>
								<p>
									<strong>Autor</strong>: <?= anchor( base_url().'autor/'.filterQuitSpecChar($author->author_fullname),$author->author_fullname) ?></p>
								<p>
								...
								<p>
									<strong>Descripción:</strong> <?= $book->book_desc ?></p>
							</div>
						</div>
						<div class="row">
							    <?php if(!$bookInList) { ?>
								<div class="col-xs-6">
								<a href="<?= base_url() ?>anadir-libro/<?= $book->id ?>"
									class="btn btn-success"> <i class="fa fa-plus"></i> Añadir a mi
									lista
								</a>
							</div>
							<div class="col-xs-6">
								<a href="<?= base_url('lista-libros') ?>"
									class="btn btn-warning disabled"
									title="Añadelo a tu lista para votar"> <i
								...
								<a href="<?= base_url() ?>quitar-libro/<?= $book->id ?>"
									class="btn btn-danger"> <i class="fa fa-minus"></i> Quitar de
									mi lista
								</a>
							</div>
							<div class="col-xs-6">
				...
				<div class="row">
						...
						<div class="panel-heading">
							<h4 class="panel-title">
								<span id="<?= $comment['num_comment'];?>"
									class="id-user-comment">#<?= $comment['num_comment'];?></span>
            ...
            ?>
						...
						<div class="panel-body">
							<a href="<?= base_url('usuario/'.$comment['user_nickname']) ?>">
								<img alt="Brand" class="commentAvatar"
								src="<?= asset_url().'images/users/'.$comment['user_avatar'] ?>">
							</a>
                                <?= $comment['text']?>
                            </div>
					</div>    						
	...
	<div class="col-xs-12">
		<div class="panel">
			...
			<div class="panel-body">
					...
					<input type="text" name="userId" value="<?= $id ?>" hidden> <input
						type="text" name="bookId" value="<?= $book->id ?>" hidden>
				</div>
				...
				<button type="submit" class="btn btn-info">
					...
					<div>
					<label for="idComment" class="control-label lead"> Inicia sesión
						para comentar </label>
				</div>
					<?php } ?>
	           </div>
