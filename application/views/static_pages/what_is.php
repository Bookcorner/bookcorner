<div class="col-xs-12">
	<div class="col-xs-6 col-xs-offset-2">
		<h2>¿Qué es Bookcorner?</h2>
		<p>Bookcorner es un sitio web dedicado única y exclusivamente a los
			libros.</p>
		<p>
			Pretende ser un lugar donde los amantes de los mismos puedan tener un
			<strong>registro organizado de todos los libros</strong> que han
			leido o quieren leer, así como poder conocer las tendencias
			principales de libros de la actualidad
		</p>
		<p>
			Está en <strong>crecimiento constante: </strong>dado que los libros
			no paran de salir, es (y será) frecuente que el sitio no disponga del
			libro que quieres añadir a tu lista en este momento. Debido a la
			cantidad ingente de libros que hay, os pedimos vuestra ayuda para
			seguir aumentando el contenido del sitio, pudiendo vosotros enviar la
			información de ese libro o de ese autor que tanto os gusta y que no
			está registrado todavía.
		</p>
		<p>
			Tampoco queremos aburriros con las características que tiene, o la
			visión a futuro del sitio, os dejamos la parte de investigación.
			Recordad que si teneis alguna sugerencia, siempre será bienvenida <a
				href="#">aquí</a>
		</p>
	</div>
</div>