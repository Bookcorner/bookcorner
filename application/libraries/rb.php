...
namespace RedBeanPHP {
     ...
     * @license BSD/GPLv2
     *         
     * @copyright copyright (c) G.J.G.T. (Gabor) de Mooij and the RedBeanPHP Community
...
namespace RedBeanPHP\Logger {
     ...
     * @license BSD/GPLv2
     *         
     * @copyright copyright (c) G.J.G.T. (Gabor) de Mooij
     *            This source file is subject to the BSD/GPLv2 License that is bundled
     *            with this source code in the file license.txt.
     */
...
namespace RedBeanPHP\Logger\RDefault {
    ...
    class Debug extends RDefault implements Logger {
         ...
         * SQL queries.
         * Set this value to a reasonable number to
...
namespace RedBeanPHP {
    ...
    interface Driver {
        ...
        /**
         * Toggles debug mode.
         * In debug mode the driver will print all
...
namespace RedBeanPHP\Driver {
     ...
     * @license BSD/GPLv2
     *         
     * @copyright copyright (c) Desfrenes & Gabor de Mooij and the RedBeanPHP community
     *            This source file is subject to the BSD/GPLv2 License that is bundled
     *            with this source code in the file license.txt.
     */
    class RPDO implements Driver {
        ...
        /**
         * Binds parameters.
         * This method binds parameters to a PDOStatement for
        ...
        protected function runQuery($sql, $bindings, $options = array()) {
            ...
            try {
                if (strpos ( 'pgsql', $this->dsn ) === 0) {
                    $statement = $this->pdo->prepare ( $sql, array (
        ...
        /**
         * Constructor.
         * You may either specify dsn, user and password or
         ...
         * functionality.
         * If a connection has already been established this
         ...
         */
        public function GetCursor($sql, $bindings = array()) {
            $statement = $this->runQuery ( $sql, $bindings, array (
                    'noFetch' => TRUE 
            ) );
...
namespace RedBeanPHP {
     ...
     * OODBBean represents a bean. RedBeanPHP uses beans
     *
     * @author Gabor de Mooij and the RedBeanPHP community
     * @license BSD/GPLv2
     * @copyright copyright (c) G.J.G.T. (Gabor) de Mooij and the RedBeanPHP Community.
    ...
    class OODBBean implements \IteratorAggregate, \ArrayAccess, \Countable {
        ...
        public static function setErrorHandlingFUSE($mode, $func = NULL) {
            if ($mode !== self::C_ERR_IGNORE && $mode !== self::C_ERR_LOG && $mode !== self::C_ERR_NOTICE && $mode !== self::C_ERR_WARN && $mode !== self::C_ERR_EXCEPTION && $mode !== self::C_ERR_FUNC && $mode !== self::C_ERR_FATAL)
        ...
        /**
         * This is where the real properties of the bean live.
         * They are stored and retrieved
        ...
        /**
         * Sets a meta property for all beans.
         * This is a quicker way to set
         ...
         *
         * @param string $type
         *            the name of the list you want to retrieve.
         *            
         * @return array
         */
        private function getSharedList($type, $redbean, $toolbox) {
            ...
            if ($this->via) {
                $oldName = $writer->getAssocTable ( array (
                        $this->__info ['type'],
                        $type 
                ) );
                if ($oldName !== $this->via) {
         ...
         *
         * @param string $type
         *            name of the list you want to retrieve
         * @param OODB $oodb
         *            The RB OODB object database instance
         *            
         * @return array
         */
        ...
        /**
         * Initializes a bean.
         * Used by OODB for dispensing beans.
         ...
         * Normally the Bean Helper is set by OODB.
        ...
        /**
         * Imports all values from an associative array $array.
         * Chainable.
        ...
        /**
         * Just like with().
         * Except that this method prepends the SQL query snippet
         ...
         * conditions.
         * If you have an ownList or sharedList with a
         ...
         * its contents.
         * Use this if you only want to add something to a list
        ...
        /**
         * Prepares an own-list to use an alias.
        ...
        public function beau($property) {
            ...
            if ((strpos ( $property, 'own' ) === 0 && ctype_upper ( substr ( $property, 3, 1 ) )) || (strpos ( $property, 'xown' ) === 0 && ctype_upper ( substr ( $property, 4, 1 ) )) || (strpos ( $property, 'shared' ) === 0 && ctype_upper ( substr ( $property, 6, 1 ) ))) {
        ...
        /**
         * Magic Getter.
         * Gets the value for a specific property in the bean.
         ...
         * If the model implements a __toString() method this
        ...
        public function hasChanged($property) {
            return (array_key_exists ( $property, $this->properties )) ? $this->old ( $property ) != $this->properties [$property] : FALSE;
        }
         ...
         *
         * @param string|OODBBean $type
         *            type of bean to dispense or the full bean
         * @param string|array $qualification
        ...
        public function equals(OODBBean $bean) {
            return ( bool ) ((( string ) $this->properties ['id'] === ( string ) $bean->properties ['id']) && (( string ) $this->__info ['type'] === ( string ) $bean->__info ['type']));
        }
    }
}
...
namespace RedBeanPHP {
     ...
     * Part of the observer pattern in RedBean
     *
     * @author Gabor de Mooij and the RedBeanPHP community
     * @license BSD/GPLv2
     * @copyright copyright (c) G.J.G.T. (Gabor) de Mooij and the RedBeanPHP Community.
    ...
    interface Observer {
         ...
         * notifications.
         * Therefore the observer needs to implement the
...
namespace RedBeanPHP {
    ...
    interface Adapter {
         ...
         * the first two columns of the resultset.
        ...
        /**
         * Returns the original database resource.
...
namespace RedBeanPHP\Adapter {
     ...
     * @license BSD/GPLv2
     *         
     * @copyright (c) copyright G.J.G.T. (Gabor) de Mooij and the RedBeanPHP community.
     *            This source file is subject to the BSD/GPLv2 License that is bundled
     *            with this source code in the file license.txt.
     */
...
namespace RedBeanPHP {
    ...
    class BeanCollection {
        ...
        public function next() {
            ...
            if ($row) {
                $beans = $this->repository->convertToBeans ( $this->type, array (
                        $row 
                ) );
...
namespace RedBeanPHP {
    ...
    interface QueryWriter {
         ...
         * This method is used to determine the minimum
        ...
        /**
         * Returns records through an intermediate type.
         * This method is used to obtain records using a link table and
         ...
         * If no primary key ID is provided, an
         ...
         * does not.
         * The purpose of this method is to translate the database specific state to
         ...
         * A database structure has to conform to the
         ...
         *
         * @param string $databaseStructure
         ...
         * For instance if you would like to refer to
...
namespace RedBeanPHP\QueryWriter {
    ...
    abstract class AQueryWriter {
        ...
        private function getRelationalTablesAndColumns($sourceType, $destType, $noQuote = FALSE) {
            $linkTable = $this->esc ( $this->getAssocTable ( array (
                    $sourceType,
                    $destType 
            ) ), $noQuote );
         ...
         *
         * @param
         *            array &$valueList list of values to generate slots for (gets modified if needed)
        ...
        /**
         * Checks whether a value starts with zeros.
         * In this case
         ...
         * and corresponding insertvalues.
         * Returns the insert id.
         *
         * @param string $table
         *            table to perform query on
        ...
        /**
         * Checks whether the specified type (i.e.
        ...
        public function updateRecord($type, $updatevalues, $id = NULL) {
            ...
            if (! $id) {
                ...
                return ( string ) $this->insertRecord ( $table, $insertcolumns, array (
                        $insertvalues 
                ) );
            }
         ...
         */
        public function queryTagged($type, $tagList, $all = FALSE, $addSql = '', $bindings = array()) {
            $assocType = $this->getAssocTable ( array (
                    $type,
                    'tag' 
            ) );
		...
		";
            ...
            $bindings = array_merge ( $tagList, array (
                    $score 
            ), $bindings );
            $rows = $this->adapter->get ( $sql, $bindings );
            return $rows;
...
namespace RedBeanPHP\QueryWriter {
    ...
    class MySQL extends AQueryWriter implements QueryWriter {
         ...
         * foreign keys.
         * Returns one if the data type constants.
         *
...
namespace RedBeanPHP\QueryWriter {
    ...
    class SQLiteT extends AQueryWriter implements QueryWriter {
         ...
         *            target type
         * @param string $field
         *            field of the type that needs to get the fk
         * @param string $targetField
         *            field where the fk needs to point to
         * @param integer $buildopt
         *            0 = NO ACTION, 1 = ON DELETE CASCADE
         *            
        ...
        public function scanType($value, $flagSpecial = FALSE) {
            ...
            if ((is_numeric ( $value ) && $value < 2147483648 && $value > - 2147483648) || preg_match ( '/\d{4}\-\d\d\-\d\d/', $value ) || preg_match ( '/\d{4}\-\d\d\-\d\d\s\d\d:\d\d:\d\d/', $value )) {
...
namespace RedBeanPHP\QueryWriter {
    ...
    class PostgreSQL extends AQueryWriter implements QueryWriter {
        ...
        public function scanType($value, $flagSpecial = FALSE) {
            ...
            if ($value === FALSE || $value === TRUE || $value === NULL || (is_numeric ( $value ) && AQueryWriter::canBeTreatedAsInt ( $value ) && $value < 2147483648 && $value > - 2147483648)) {
                return self::C_DATATYPE_INTEGER;
...
namespace RedBeanPHP {
    ...
    abstract class Repository {
        ...
        /**
         * Process groups.
         * Internal function. Processes different kind of groups for
         ...
         *
         * @param OODBBean|SimpleModel $embeddedBean
         *            the bean or model
         *            
         * @return integer
         */
         ...
         *
         * @param OODBBean|SimpleModel $bean
         *            bean to store
         *            
         ...
         * Pass a type and a series of ids and
        ...
        public function batch($type, $ids) {
            ...
            $collection = array ();
            try {
                $rows = $this->writer->queryRecord ( $type, array (
                        'id' => $ids 
                ) );
            } catch ( SQLException $e ) {
         ...
         * (arrays) into beans.
         * Given a type and a set of rows this method
        ...
        public function trash($bean) {
            ...
            try {
                $this->writer->deleteRecord ( $bean->getMeta ( 'type' ), array (
                        'id' => array (
                                $bean->id 
                        ) 
                ), NULL );
...
namespace RedBeanPHP\Repository {
    ...
    class Fluid extends Repository {
         ...
         *
         * @param OODBBean $bean
         *            bean to update report of
         * @param string $table
         *            table to check and create if not exists
         *            
         * @return void
         */
         ...
         *
         * @param OODBBean $bean
         *            bean to get cast data from and store meta in
         * @param string $property
         *            property to store
         * @param mixed $value
         *            value to store
        ...
        /**
         * Stores a cleaned bean; i.e.
         * only scalar values. This is the core of the store()
        ...
        /**
         * Handles\Exceptions.
         * Suppresses exceptions caused by missing structures.
         *
...
namespace RedBeanPHP {
    ...
    class OODB extends Observable {
         ...
         * more.
         *
         * @param AssociationManager $assoc
...
namespace RedBeanPHP {
    ...
    class Finder {
         ...
         * query as its resource.
         * Given an SQL query like 'SELECT movie.*, review.* FROM movie... JOIN review'
...
namespace RedBeanPHP {
    ...
    class AssociationManager extends Observable {
        ...
        private function handleException(\Exception $exception) {
            if ($this->oodb->isFrozen () || ! $this->writer->sqlStateIn ( $exception->getSQLState (), array (
                    QueryWriter::C_SQLSTATE_NO_SUCH_TABLE,
                    QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN 
            ) )) {
                throw $exception;
        ...
        /**
         * Associates a pair of beans.
         * This method associates two beans, no matter
         ...
         *
         * @param OODBBean $bean1
         *            first bean
         * @param OODBBean $bean2
         *            second bean
         * @param OODBBean $bean
        ...
        protected function associateBeans(OODBBean $bean1, OODBBean $bean2, OODBBean $bean) {
            ...
            $bean->setMeta ( 'sys.buildcommand.unique', array (
                    $property1,
                    $property2 
            ) );
         ...
         *
         * @param OODBBean|array $beans1
         *            one or more beans to form the association
         * @param OODBBean|array $beans2
         *            one or more beans to form the association
         *            
         * @return array
         */
         ...
         * This method unassociates two beans. If the
        ...
        public function unassociate($beans1, $beans2, $fast = NULL) {
            $beans1 = (! is_array ( $beans1 )) ? array (
                    $beans1 
            ) : $beans1;
            $beans2 = (! is_array ( $beans2 )) ? array (
                    $beans2 
            ) : $beans2;
            ...
            foreach ( $beans1 as $bean1 ) {
                foreach ( $beans2 as $bean2 ) {
                    try {
                        ...
                        $linkType = $this->getTable ( array (
                                $type1,
                                $type2 
                        ) );
                        
                        if ($fast) {
                            $this->writer->deleteRecord ( $linkType, array (
                                    'id' => $row ['id'] 
                            ) );
                        ...
                        $beans = $this->oodb->convertToBeans ( $linkType, array (
                                $row 
                        ) );
         ...
         * This method breaks every connection between
         ...
         * @param boolean $glue
         *            whether the SQL should be prefixed with WHERE
         *            
         * @return array
         */
...
namespace RedBeanPHP {
    ...
    class SimpleModelHelper implements Observer {
        ...
        /**
         * Attaches the FUSE event listeners.
         * Now the Model Helper will listen for
...
namespace RedBeanPHP {
    ...
    class TagManager {
        ...
        protected function findTagByTitle($title) {
            $beans = $this->redbean->find ( 'tag', array (
                    'title' => array (
                            $title 
                    ) 
            ) );
         ...
         * of the listed tags.
         * If the third parameter is TRUE this method
        ...
        /**
         * Removes all sepcified tags from the bean.
...
namespace RedBeanPHP {
    ...
    class LabelMaker {
        ...
        /**
         * Gathers labels from beans.
         * This function loops through the beans,
        ...
        public function enum($enum) {
            ...
            $newEnumItems = $this->dispenseLabels ( $type, array (
                    $value 
            ) );
...
namespace RedBeanPHP {
    ...
    class Facade {
        ...
        /**
         * Internal Query function, executes the desired query.
         * Used by
        ...
        private static function query($method, $sql, $bindings) {
            if (! self::$redbean->isFrozen ()) {
                try {
                    ...
                    if (self::$writer->sqlStateIn ( $exception->getSQLState (), array (
                            QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN,
                            QueryWriter::C_SQLSTATE_NO_SUCH_TABLE 
                    ) )) {
         ...
         * This method should be called before you start using
         ...
         * If you pass the type of a bean this
         ...
         * and THEN trash it.
         *
         * @param string|OODBBean|SimpleModel $bean
         *            bean you want to remove from database
         * @param integer $id
         *            (optional)
         *            
         ...
         *
         * @param string|array $typeOrBeanArray
        ...
        /**
         *
         * @see Facade::find The findAll() method differs from the find() method in that it does
        ...
        /**
         *
         * @see Facade::find The variation also exports the beans (i.e. it returns arrays).
         *     
         * @param string $type
         *            type the type of bean you are looking for
         * @param string $sql
        ...
        /**
         *
         * @see Facade::find This variation returns the first bean only.
         *     
         * @param string $type
         *            type the type of bean you are looking for
         * @param string $sql
        ...
        /**
         *
         * @see Facade::find This variation returns the last bean only.
         *     
         * @param string $type
         *            type the type of bean you are looking for
         * @param string $sql
         ...
         * This is a very powerful yet complex function.
         ...
         * @param string|array $sqlOrArr
         *            SQL query string or result set array
         * @param array $bindings
         *            SQL bindings
         * @param array $remappings
        ...
        /**
         *
         * @see Facade::batch Alias for batch(). Batch method is older but since we added so-called *All
         ...
         * @param array $bindings
         *            values a list of values to be bound to query parameters
         *            
         * @return integer
         */
         ...
         * functionality.
         * Alias for R::getAdapter()->getInsertID().
         *
         * @return mixed
         */
         ...
         *
         * @deprecated This function is deprecated in favour of R::duplicate().
         ...
         * Handy for XML/JSON exports with a
         ...
         * For information please consult the R::store()
         * function.
         * A loop saver.
         *
         * @param array $beans
        ...
        /**
         * Generates and returns an ENUM value.
         * This is how RedBeanPHP handles ENUMs.
         ...
         * criteria set.
         * If no beans are found a new bean
        ...
        /**
         * Call static for use with dynamic plugins.
...
namespace RedBeanPHP {
    ...
    class DuplicationManager {
        ...
        /**
         * Copies the shared beans in a bean, i.e.
         * all the sharedBean-lists.
         *
         * @param OODBBean $copy
         *            target bean to copy lists to
         * @param string $shared
         *            name of the shared list
         * @param array $beans
         *            array with shared beans to copy
         *            
         * @return void
         */
        ...
        /**
         * Copies the own beans in a bean, i.e.
         * all the ownBean-lists.
         ...
         *            target bean to copy lists to
         * @param string $owned
         *            name of the own list
         * @param array $beans
         *            array with shared beans to copy
         * @param array $trail
         *            array with former beans to detect recursion
         * @param boolean $preserveIDs
         *            TRUE means preserve IDs, for export only
         *            
         * @return void
         */
         ...
         * and the parents beans to the newly created bean.
         * Also sets the ID of the bean
         * to 0.
         *
         * @param OODBBean $bean
         *            bean to copy
        ...
        protected function hasSharedList($type, $target) {
            return in_array ( AQueryWriter::getAssocTableFormat ( array (
                    $type,
                    $target 
            ) ), $this->tables );
        }
...
namespace {
     ...
     *            enum code as you would pass to R::enum()
     *            
     * @return mixed
     */
